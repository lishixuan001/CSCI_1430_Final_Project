...
class Model(object):
    ...
    def __init__(self, dataset, model_name, n_classes, max_n_objects,
        ...
        assert self.dataset in ['CVPPP', "meibo"]
        assert self.model_name in ['ReSeg', 'StackedRecurrentHourglass']

        if self.dataset in ['CVPPP', "meibo"]:
...
#         print(cpu_images.shape) # (8, 3, 256, 256)
        
#         print(type(cpu_sem_seg_annotations))
#         print(cpu_sem_seg_annotations.shape) # (8, 2, 256, 256)
        
#         print(type(cpu_ins_seg_annotations))
#         print(cpu_ins_seg_annotations.shape) # (8, 20, 256, 256)
        
#         print(type(cpu_n_objects))
#         print(cpu_n_objects) # tensor([15, 20, 19, 17, 15, 15, 20, 15], dtype=torch.int32)
...
#         instances = cpu_ins_seg_annotations[0]
#         for i in range(instances.shape[0]):
#             instance = instances[i]
#             print(instance.shape)
#             print(torch.max(instance))
        
#         print(type(cpu_images))
#         print(cpu_images.shape)
        
#         print(type(cpu_sem_seg_annotations))
#         print(cpu_sem_seg_annotations.shape)
        
#         print(type(cpu_ins_seg_annotations))
#         print(cpu_ins_seg_annotations.shape)
...
#         print("=============================================")
    ...
    def __test(self, test_loader, criterion_type, epoch, debug):
        ...
        test_metric_vis_data = [float(num) for num in test_metric_vis_data]
    ...
    def fit(self, criterion_type, delta_var, delta_dist, norm,
        ...
        for epoch in range(n_epochs):
            ...
            training_metric_vis_data = [float(num) for num in training_metric_vis_data]
            ...
            if is_best_model:
                ...
                torch.save(self.model.state_dict(), os.path.join(
                    model_save_path, 'model_{}_{}.pth'.format(str(epoch).zfill(len(str(n_epochs))),
                                                              val_cost)))
